...
public class SqlStatement {
	...
	public static String getTeamOpponentSum(String date, String opponent) {
		String r = "SELECT SUM(fieldGoal),SUM(fieldGoalAttempts),SUM(freeThrowAttempts),SUM(offensiveRebound),SUM(defensiveRebound),SUM(turnOver),SUM(scoring),SUM(backBoard),SUM(threePointFieldGoalAttempts) FROM playerdata WHERE date='"
	...
	public static String countPlayerMatches(String playerName, String season) {
		String r = "SELECT COUNT(playername) FROM playerdata WHERE playername='"
	...
	public static String getPlayerFirstPlay(String playerName, String season) {
		String r = "SELECT COUNT(*) FROM playerdata WHERE position !=''AND playername='"
				+ playerName + "' AND " + season;
		return r;
	}

	public static String getPlayerTeam(String playerName) {
		String r = "SELECT team FROM playerdata WHERE playername='"
				+ playerName + "' ORDER BY date DESC LIMIT 1";
		return r;
	}

	public static String countTeamSumForPlayer(String teamName, String season) {
		String r = "SELECT SUM(fieldGoal),SUM(fieldGoalAttempts),SUM(freeThrow),SUM(offensiveRebound),SUM(defensiveRebound),SUM(minutes),SUM(freeThrowAttempts),SUM(backboard),SUM(turnOver) FROM playerdata WHERE team='"
				+ teamName + "' AND " + season;
		return r;
	}

	public static String getOpponentSumForPlayer(String date, String opponent) {
		String r = "SELECT SUM(fieldGoalAttempts),SUM(threePointFieldGoalAttempts),SUM(backBoard),SUM(offensiveRebound),SUM(defensiveRebound) FROM playerdata WHERE date='"
				+ date + "' AND team='" + opponent + "'";
		return r;
	}
}
