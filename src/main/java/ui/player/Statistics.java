...
public class Statistics extends MyPanel implements ActionListener {
	...
	String[] columnNames = { "球员名称", "所属球队", "参赛场数", "先发场数", "篮板数", "助攻数",
			"在场时间", "投篮命中率", "三分命中率", "罚球命中率", "进攻数", "防守数", "抢断数", "盖帽数",
			"失误数", "犯规数", "得分", "效率", "GmSc效率值", "真实命中率", "投篮效率", "篮板率",
			"进攻篮板率", "防守篮板率", "助攻率", "抢断率", "盖帽率", "失误率", "使用率", "场均得分",
			"场均时间", "场均篮板", "场均助攻", "场均投篮命中数", "场均投篮出手数", "场均三分命中数", "场均三分出手数",
			"场均罚球命中数", "场均罚球出手数", "场均进攻数", "场均防守数", "场均抢断数", "场均盖帽数", "场均失误数",
			"场均犯规数" };
	String[] columnNames1 = { "姓名", "对手", "上场时间", "得分", "投篮命中数", "投篮出手数",
	...
	public Statistics(Frame frame) {
		...
		Object[][] data = null/*
							 * getData(prs.getAllPlayerdata("13-14","scoring",
							 * "DESC"))
							 */;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("home")
			...
			Object[][] data = getData(prs.getAllPlayerdata("13-14",
			...
			if (hot.getSelectedIndex() == 0) {
				...
				Object[][] data = getData(prs.getSeasonTop("13-14", Translate
						.translate1(term1.getSelectedItem().toString())));
				model.setDataVector(data, columnNames);
				table.setWidth();
				table.updateUI();
