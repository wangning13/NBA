...
public class TeamRank implements TeamRankService {
	...
	private ArrayList<TeamPO> Ranking(String season, String condition,
			String order) {
	...
	private ArrayList<TeamVO> gettingTeamData(String season, String condition,
			String key, String order) {
		ArrayList<TeamVO> teamVOs = new ArrayList<TeamVO>();
		ArrayList<TeamPO> teamPOs2 = null;
		GetTeamdataDataService g;
		g = new GetTeamdata();
		teamPOs2 = g.getByEfficiency(this.Ranking(season, condition, order),
				key, order);
	...
	public ArrayList<TeamVO> getTeamData(String season, String condition,
			String key, String order) {
		ArrayList<TeamVO> teamVOs = gettingTeamData(season, condition, key,
				order);
		return teamVOs;
	}
	...
	public TeaminfoVO getTeamInfo(String teamName) {
		...
		teaminfoVO = new TeaminfoVO(teaminfoPO.getName(), teaminfoPO.getAbbr(),
				teaminfoPO.getCity(), teaminfoPO.getLeague(),
				teaminfoPO.getPartition(), teaminfoPO.getCourt(),
				teaminfoPO.getYear());
	...
	public ArrayList<TeamMonthMatchVO> getTeamMonthMatch(String month,
		...
		for (int i = 0; i < teamMatchPOs.size(); i++) {
			ArrayList<TeamMatchPO> teamMatchPOs2 = g.getTeamMonthMatch(month,
			...
			for (int j = 0; j < teamMatchPOs2.size(); j++) {
				if (teamMatchPOs2.get(j).getOpponent().equals(team)) {
					...
					String score = teamMatchPOs.get(i).getTotal() + "-"
							+ teamMatchPOs2.get(j).getTotal();
					String first = teamMatchPOs.get(i).getFirst() + "-"
							+ teamMatchPOs2.get(j).getFirst();
					String second = teamMatchPOs.get(i).getSecond() + "-"
							+ teamMatchPOs2.get(j).getSecond();
					String third = teamMatchPOs.get(i).getThird() + "-"
							+ teamMatchPOs2.get(j).getThird();
					String fourth = teamMatchPOs.get(i).getFourth() + "-"
							+ teamMatchPOs2.get(j).getFourth();
					TeamMonthMatchVO teamMonthMatchVO = new TeamMonthMatchVO(
							data, host, guest, score, first, second, third,
							fourth);
	...
	public TeamMatchVO getTeamMatch(String date, String team) {
		...
		teamMatchVO = new TeamMatchVO(teamMatchPO.getDate(),
				teamMatchPO.getHostGuest(), teamMatchPO.getName(),
				teamMatchPO.getOpponent(), teamMatchPO.getWinLose(),
				teamMatchPO.getTotal(), teamMatchPO.getFirst(),
				teamMatchPO.getSecond(), teamMatchPO.getThird(),
	...
	public ArrayList<TeamMonthMatchVO> getTeamRecentFiveMatch(String team) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (teamMatchPOs.get(i).getHostGuest().equals("h")) {
				...
				score = teamMatchPOs.get(i).getTotal() + "-"
						+ teamMatchPOs.get(i + 5).getTotal();
				first = teamMatchPOs.get(i).getFirst() + "-"
						+ teamMatchPOs.get(i + 5).getFirst();
				second = teamMatchPOs.get(i).getSecond() + "-"
						+ teamMatchPOs.get(i + 5).getSecond();
				third = teamMatchPOs.get(i).getThird() + "-"
						+ teamMatchPOs.get(i + 5).getThird();
				fourth = teamMatchPOs.get(i).getFourth() + "-"
						+ teamMatchPOs.get(i + 5).getFourth();
			} else {
				host = teamMatchPOs.get(i).getOpponent();
				guest = team;
				score = teamMatchPOs.get(i + 5).getTotal() + "-"
						+ teamMatchPOs.get(i).getTotal();
				first = teamMatchPOs.get(i + 5).getFirst() + "-"
						+ teamMatchPOs.get(i).getFirst();
				second = teamMatchPOs.get(i + 5).getSecond() + "-"
						+ teamMatchPOs.get(i).getSecond();
				third = teamMatchPOs.get(i + 5).getThird() + "-"
						+ teamMatchPOs.get(i).getThird();
				fourth = teamMatchPOs.get(i + 5).getFourth() + "-"
						+ teamMatchPOs.get(i).getFourth();
			}
			TeamMonthMatchVO teamMonthMatchVO = new TeamMonthMatchVO(
					teamMatchPOs.get(i).getDate(), host, guest, score, first,
					second, third, fourth);
			teamMonthMatchVOs.add(teamMonthMatchVO);
		}
		return teamMonthMatchVOs;
