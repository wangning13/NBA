...
public class PlayerRank implements PlayerRankService {
	...
	public PlayerinfoVO getPlayerinfo(String playerName) {
		...
		if (playerinfoPO == null) {
			playerinfoVO = new PlayerinfoVO("", "", "", "", 0, "", 0, "", "");
		} else {
			playerinfoVO = new PlayerinfoVO(playerinfoPO.getName(),
					playerinfoPO.getNumber(), playerinfoPO.getPosition(),
					playerinfoPO.getHeight(), playerinfoPO.getWeight(),
					playerinfoPO.getBirth(), playerinfoPO.getAge(),
					playerinfoPO.getExp(), playerinfoPO.getSchool());
		}
		return playerinfoVO;
	}

	public ArrayList<PlayerVO> getAllPlayerdata(String season, String key,
			String order) {
		...
		ArrayList<PlayerPO> playerPOs2 = g.getByEfficiency(playerPOs, key,
				order);
	...
	public ArrayList<PlayerVO> getFirstFifty(String season, String position,
		...
		if (key.equals("weightAverage")) {
			playerPOs = g.getSomePlayerdata(season, position, partition,
					"scoring", "DESC");
			for (int i = 0; i < playerPOs.size() - 1; i++) {
				for (int j = 0; j < playerPOs.size() - i - 1; j++) {
					if (playerPOs.get(j).getScoring()
							+ playerPOs.get(j).getBackboard()
							+ playerPOs.get(j).getAssist() < playerPOs.get(
							j + 1).getScoring()
							+ playerPOs.get(j + 1).getBackboard()
							+ playerPOs.get(j + 1).getAssist()) {
	...
	public ArrayList<PlayerMatchVO> getPlayerMatchdata(String date, String team) {
		...
		for (int i = 0; i < playerMatchPOs.size(); i++) {
			...
			PlayerMatchVO playerMatchVO = getPlayerMatchVO
					.getPlayerMatchVO(playerMatchPOs.get(i));
